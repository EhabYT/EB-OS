  ...
  Function to add and set the EB OS themes by default
function Set-EBTheme {
...
function Remove-ExtractFromContextMenu {
    & "$windir\EBDesktop\4. Interface Tweaks\Context Menus\Extract\Remove Extract (default).cmd" /justcontext
}

  Function to remove printing from context menus
function Remove-PrintingFromContextMenus {
   & "$windir\EBDesktop\6. Advanced Configuration\Services\Printing\Disable Printing.cmd" /justcontext
}
...
function Set-SendToContextMenu {
    & "$windir\EBDesktop\4. Interface Tweaks\Context Menus\Send To\Debloat Send To Context Menu.cmd" -Disable @('Documents', 'Mail Recipient', 'Fax recipient', 'Bluetooth')
}
...
function Hide-GalleryInFileExplorer {
    & "$windir\EBDesktop\4. Interface Tweaks\File Explorer Customization\Gallery\Disable Gallery (default).cmd" /justcontext
}
...
function Disable-NetworkNavigationPaneInExplorer {
    & "$windir\EBDesktop\3. General Configuration\File Sharing\Network Navigation Pane\Disable Network Navigation Pane (default).cmd" /justcontext
}
...
function Restore-OldContextMenu {
    & "$windir\EBDesktop\4. Interface Tweaks\Context Menus\Windows 11\Old Context Menu (default).cmd" /justcontext
}
...
function Add-NetworkSharingShortcut {
    ...
    New-Shortcut -Source 'control.exe' `
        -Destination "$([Environment]::GetFolderPath('Windows'))\EBDesktop\3. General Configuration\File Sharing\Sharing Settings.lnk" `
...
function Set-StartMenu {
    ...
    foreach ($userKey in (Get-RegUserPaths).PsPath) {
        ...
        if ([string]::IsNullOrEmpty($appData) -or !(Test-Path $appData)) {
            ...
            Copy-Item -Path "$windir\EBModules\Other\Layout.xml" -Destination "$appdata\Microsoft\Windows\Shell\LayoutModification.xml" -Force
...
function Disable-AutomaticStoreAppArchiving {
    & "$windir\EBDesktop\3. General Configuration\Store App Archiving\Disable Store App Archiving (default).cmd" /justcontext
}
