...
# Backup EB OS services and drivers
$windir = $([Environment]::GetFolderPath('Windows'))
function Backup-EBServices {
$filePath = "$([Environment]::GetFolderPath('Windows'))\EBModules\Other\atlasServices.reg"
...
& "$windir\EBModules\Scripts\settingsPages.cmd" /hide account
...
function Disable-CoreIsolation {
    & ".\EBModules\Scripts\ScriptWrappers\ConfigVBS.ps1" -DisableAllVBS
}
...
function Disable-Mitigations {
    Start-Process -FilePath "EBDesktop\7. Security\Mitigations\Disable All Mitigations.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
}
...
# Set EB OS default profile pictures
...
function Set-PowerSettings {
    ...
    if ($DisablePowerSaving) {
        Start-Process -FilePath "EBDesktop\3. General Configuration\Power-saving\Disable Power-saving.cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }

    if ($DisableHibernation) {
        Start-Process -FilePath "EBDesktop\3. General Configuration\Hibernation\Disable Hibernation (default).cmd" -ArgumentList "/silent" -NoNewWindow -Wait
    }
