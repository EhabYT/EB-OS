...
$networkDiscoveryConfigPath = "$([Environment]::GetFolderPath('Windows'))\EBDesktop\6. Advanced Configuration\Services\Network Discovery"
...
if ($LASTEXITCODE -eq 1) {
    reg import "$([Environment]::GetFolderPath('Windows'))\EBDesktop\3. General Configuration\File Sharing\Network Navigation Pane\User Network Navigation Pane choice.reg" | Out-Null
}

choice /c:yn /n /m "Would you like to restore the 'Give access to' context menu in Explorer? [Y/N] "
if ($LASTEXITCODE -eq 1) {
    reg import "$([Environment]::GetFolderPath('Windows'))\EBDesktop\3. General Configuration\File Sharing\Give Access To Menu\Enable Give Access To Menu.reg" | Out-Null
}
