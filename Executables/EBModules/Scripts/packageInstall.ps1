...
param (
	...
	[string]$PackagesPath = "$([Environment]::GetFolderPath('Windows'))\EBModules\Packages",
...
"@
	...
	if ($failedPackages.Count -gt 0) {
		...
		if ($NoInteraction) {
			...
			$failedMsgTitle = 'EBOSFailedComponentMsgBox'
			$failedMsgArgs = "/c title Finalizing Installation - EB OS & echo Do not close this window. & schtasks /delete /tn `"$failedMsgTitle`" /f > nul & " `
		...
		if ($safeModeStatus) {
			Write-Host "Please report this to the EB OS team, as there's no automatic fallbacks past Safe Mode." -ForegroundColor Magenta
...
if ($SafeMode) {
	...
	if ($matchedPackages.Count -le 0) {
		Write-Host "[ERROR] Safe Mode package list not found! Please report this to EB OS." -ForegroundColor Red
		ExitSafeModePrompt
	}

	$packagesThatDontExist = $matchedPackages | ForEach-Object { if (!(Test-Path $_ -PathType Leaf)) { $_ } }
	if ($packagesThatDontExist) {
		Write-Host "[ERROR] Some Safe Mode packages weren't found. Please report this to EB OS." -ForegroundColor Red
...
Would you like EB OS to restart your system into Safe Mode and try again? This process shouldn't take much time.

Please note that if you chose to disable Windows Defender, it may still remain enabled if you select 'No'. However, you can always try disabling it later in the EB OS folder.
"@

	if ((Read-MessageBox -Title "EB OS - Component Modification" -Body $body -Icon Question) -eq 'Yes') {
		SafeMode -Enable
		Restart
	}
...
function MakeRepairSource {
	...
	$srcPath = "%SystemRoot%\EBModules\Packages\WinSxS"
